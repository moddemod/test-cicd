...
headers = {
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Safari/537.77'
}
headers_form = {
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.83 Safari/537.77',
...
def init_ui():
    if platform.system() == 'Windows':
        os.system('chcp 65001 2>nul 1>nul')
        os.system('title 假期疫情信息收集 blog:moddemod.blog.csdn.net')
        os.system('mode con: cols=80 lines=25 1>nul 2>nul')
        os.system('color 7a')
    print('''
     #    #   ####   #####   #####   ######  #    #   ####   #####
     ##  ##  #    #  #    #  #    #  #       ##  ##  #    #  #    #
...
def auto_submit():
    ...
    with open(file_name, 'r', encoding='utf-8') as f:
        ...
        while True:
            ...
            if ok:
                pre_post()
                if fill_form(username=username, address=address):
                    print('提交成功！')
                else:
                    return
                query_fill_state()
...
def submit():
    ...
    if ok:
        ...
        is_yes_no = input('是否需要需要重新填写呢？需要按Y继续，不需要按N退出').lower()
        if is_yes_no == 'n':
            return
        address = input('[+]address:')
        fill_form(username=username, address=address)
